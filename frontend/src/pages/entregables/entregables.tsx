    ...
    <div className="page">
      ...
      <div className="grid-2">
        <div className="panel">
          ...
          {estado?.ppt && <a className="detail" href={`/api/expedientes/${ref}/salidas/${estado.ppt.nombre}`}>Descargar la última versión</a>}
          <div><button className="btn btn--primary" onClick={exportar} disabled={!listo || ocupado !== ""}>{ocupado === "ppt" ? <><span className="spinner" />Exportando…</> : "Exportar informe a PowerPoint"}</button></div>
        ...
        <div className="panel">
          ...
          <div><button className="btn btn--primary" onClick={archivar} disabled={ocupado !== ""}>{ocupado === "zip" ? <><span className="spinner" />Archivando…</> : "Archivar"}</button></div>
